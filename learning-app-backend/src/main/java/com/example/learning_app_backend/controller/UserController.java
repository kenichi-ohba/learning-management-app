...
public class UserController {
  ...
  public ResponseEntity<?> registerUser(@Valid @RequestBody UserRegistrationDto registrationDto) {
    ...
    try {
      ...
      return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("ユーザー登録中にエラーが発生しました。"); // 500 Internal Server Error
    }
  ...
  public ResponseEntity<?> loginUser(@RequestBody LoginRequestDto loginRequestDto) {
    try {
      ...
      return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body("ログインに失敗しました: ユーザー名またはパスワードが正しくありません。");
    } catch (Exception e) {
