  ...
  useEffect(() => {
    const fetchDashboardSummary = async () =>{
      ...
      try {
        ...
        console.log ('HomePage: Dashboard summary data received:', response.data);
        setSummaryData(response.data);
      } catch (err) {
        console.error('HomePage: ダッシュボードデータの取得に失敗しました:', err);
        if (err.response) {
          setError(`データの読み込みに失敗しました (Status: ${err.response.status})。再ログインが必要かもしれません。`);
        } else {
  ...
  return (
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
      ...
      <div className="bg-white p-6 rounded-lg shadow">
        <h2 className="text-lg font-semibold mb-2 text-gray-500">今週の合計学習時間</h2>
      ...
      {/* カード 2: 今週の記録数 */}
      <div className="bg-white p-6 rounded-lg shadow">
        <h2 className="text-lg font-semibold mb-2 text-gray-500">今週の記録数</h2>
        <p className="text-3xl font-bold text-green-600">
          {summaryData.recordCountThisWeek || 0}
          <span className="text-xl">件</span>
        </p>
      </div>
      {/* カード 3: (将来的に目標機能を追加した場合のプレースホルダー) */}
      <div className="bg-white p-6 rounded-lg shadow">
         <h2 className="text-lg font-semibold mb-2 text-gray-500">未達成の目標</h2>
         <p className="text-3xl font-bold text-yellow-600">?</p> {/* 仮表示 */}
         {/* <Link to="/goals" className="text-sm text-blue-600 hover:underline mt-2 inline-block">目標管理へ &rarr;</Link> */}
      </div>
    </div>
    ...
    <div className="bg-white p-6 rounded-lg shadow mb-8">
      <div className="flex justify-between items-center mb-4">
        ...
        <Link to="/records" className="text-sm text-blue-600 hover:underline">すべて表示</Link>
      </div>
      <div className="space-y-4">
        {summaryData.recentRecords && summaryData.recentRecords.length > 0 ? (
          summaryData.recentRecords.map((record) => (
            <div key={record.recordId} className="border-b pb-3 last:border-b-0">
              <p className="text-sm text-gray-500">
                {record.recordDate}
                <span className="ml-2 text-gray-400">({record.durationMinutes}分)</span>
              </p>
